...
async function runVerification() {
  ...
  await storePaymentIntent(rzpOrderIdA, {
    ...
    checkout_payload: {
      ...
      line_items: [{ product_id: 232, quantity: 1 }], // Product 232 has stock = 7
    },
  });
  ...
  printTable(
    ...
    `SELECT rzp_order_id, status, rzp_payment_id, wc_order_id, error_reason FROM wp_mumbai_payment_intents WHERE rzp_order_id = '${rzpOrderIdA}';`
  );
  ...
  // Product 225 has stock = 0 in database
  ...
  let refundRecordedId = `rfnd_${Date.now()}`;
  _setRazorpayInstanceForTesting({
    payments: {
      fetch: async () => ({
        id: rzpPaymentIdB,
  ...
  // Run 1: First webhook delivery (creates order)
  ...
  // Run 2: Replay webhook (idempotent success)
  ...
  console.log("Case E Outage Outcome (must NOT be refunded, status 502):", resEOutage);
  ...
  // Release locks before running reconciler
  await releaseLock("cron_reconciler_worker");
  await releaseLock(rzpOrderIdE);

  // Run reconciler to heal the transaction
  console.log("\n[Reconciler] Running background payment reconciler to heal 'paid' intent...");
  const reconResult = await runPaymentReconciliation({ olderThanMinutes: 0 });
